...
class utils extends Factory
{
    ...
    public function getMainPrompt(): string
    {
        ...
        $prompts[] = '- **读取**：';
        $prompts[] = '  1. 新会话先读`misc`（限当前会话）10条建立上下文；不足则用偏移量跳跃采样（如取总数1/2、1/3位置），避免只读取开头部分；最多3次；无记录则跳过。';
        $prompts[] = '  2. 用户提及具体日期或需某天完整对话流时，优先按日期读`misc`（原文）和`daily`（总结），当日为空则向前回溯1-2天。';
        $prompts[] = '  3. 上述仍不足则转入"搜索"。';
        $prompts[] = '  4. 相同条件禁止重复读取。';

        $prompts[] = '- **搜索**：上下文不足时使用。逐层递进，命中即止（不跳级、不并行）。';
        $prompts[] = '  1. 先搜`misc`（限当前会话）。';
        $prompts[] = '  2. 再搜`daily`（限当前会话）。';
        $prompts[] = '  3. 仍无则搜`important`（不限会话）。';
        $prompts[] = '  4. 最后合并搜索`all`（限当前会话）。';
        $prompts[] = '  5. 关键词选1-5个最具辨识度的特征词（避免泛化）；结果过多时采用跳跃式采样，避免只取开头几条，可设置不同偏移量并灵活调整时间跨度；多词可全部命中或任一命中；整链最多换词5次，仍无则回复“记忆中无相关记录”。';
        $prompts[] = '  6. 命中后需补全上下文，按命中日期读取当日的`misc`和`daily`（与读取规则一致），不足再继续搜索。';
        $prompts[] = '  7. 相同条件禁止重复搜索。';
